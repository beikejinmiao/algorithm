...
def _to_list(text):
    if isinstance(text, (list, tuple)):
        return [list(map(int, item)) for item in text]
...
def _target_in_deadends(target, deadends):
    arrive = False
    for j in range(len(target)):
        neighbors = _neighbors(target[j])
        for x in neighbors:
            _node = target.copy()
            _node[j] = x
            arrive |= _node not in deadends
    return arrive
